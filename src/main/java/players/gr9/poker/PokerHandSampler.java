...
public class PokerHandSampler {
    ...
    public List<FrenchCard> sampleOpponentCards(PokerGameState state, int opponentIdx) {
        ...
        logger.log(Level.WARNING, "Total hands sampled: {0} for player {1} ",new Object[] {result.size(), opponentIdx});
        return result;
    }
    ...
    private List<FrenchCard> sampleBucketHands(int bucketIndex, int numSamples) {
        ...
        if (bucketHands.isEmpty()) {
            logger.log(Level.WARNING, "Bucket {0} is empty.", bucketIndex);
        ...
        if (bucketHands.size() <= sampleSize) {
            logger.log(Level.WARNING, "Bucket {0} has <= sampleSize ({1}) hands, returning all.", new Object[]{bucketIndex, sampleSize});
        ...
        logger.log(Level.WARNING, "Sampled {0} hands from bucket {1}", new Object[]{sampleSize, bucketIndex});
        logger.log(Level.WARNING, "Chosen determinisation for player: "+ selected);
        return selected;
    ...
    private boolean isBoardWet(Deck<FrenchCard> communityCards) {
        ...
        logger.log(Level.WARNING, "Board wetness check: flushDraw={0}, straightDraw={1}", new Object[]{hasFlushDraw, hasStraightDraw});
    ...
    private boolean isAggressive(PokerGameState state, int playerIdx) {
        ...
        logger.log(Level.WARNING, "Player {0} aggression: {1}", new Object[]{playerIdx, result});
        return result;
    }
}