...
//    private boolean considerHoleCards = false; // Default value: when generating hand buckets we donot consider the hole cards
//
...
//        //List<FrenchCard> communityCardsDeck = (List<FrenchCard>) gameState.getCommunityCards().copy();
//        Deck<FrenchCard> fullDeck = FrenchCard.generateDeck("Full Deck", core.CoreConstants.VisibilityMode.VISIBLE_TO_ALL); //create a complete 52-card deck
//        List<FrenchCard> allRemainingCards = new ArrayList<>(fullDeck.getComponents());
//        allRemainingCards.removeAll(gameState.getCommunityCards().getComponents()); // Remove community cards
//
...
//            if ((kickerValue != 14) &(kickerValue > topCardValue)) { //if Ace is highest on the board ignore for simplicity
...
//                for (FrenchCard.Suite suit1 : FrenchCard.Suite.values()) {
//                    for (FrenchCard.Suite suit2 : FrenchCard.Suite.values()) {
//                        // ordinal check is to avoid creating the same hand twice but with the cards in a different order
//                        if ((suit1.ordinal() < suit2.ordinal()) && (suit1 != topCard.suite )){ // and also exclude the top card's suit
//                            List<FrenchCard> hand = new ArrayList<>();
//                            hand.add(new FrenchCard(topCard.type, suit1));
//                            hand.add(new FrenchCard(kickerType.type, suit2));
//                            tpgkHands.add(hand);
...
//    }
//
//    //TODO: add more buckets and review ^ current implementations
//
